...
export function CalculatedField({
    ...
    return (
      <div className={`flex items-center justify-between gap-2 ${className}`}>
        <div className="min-w-0">
          <p className="text-xs text-gray-600 truncate">{field.label}</p>
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <FormulaHint formula={field.formula} excelRef={field.excelRef} />
        </div>
      </div>
    )
  }
  ...
  return (
    <div className={`rounded-xl bg-slate-50 border border-slate-100 px-3 py-2.5 ${className}`}>
      ...
      <p className="text-[10px] text-gray-400 mt-1 truncate" title={field.formula}>
        {field.formula}
      </p>
    </div>
  )
