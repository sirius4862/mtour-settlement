-- settlement workflow v1 — C3: master paid-settlement lock
-- Authority: docs/workflow_decision_v1.md
-- Run AFTER settlement_workflow_v1_p0_enforce_and_admin_rls.sql on staging first.
--
-- Layer 1: enforce_settlement_workflow — master paid lock + deny-default transitions
-- Layer 2: settlements_master_admin_update (non-paid) + settlements_master_reopen_paid
-- Safe to re-run: CREATE OR REPLACE + DROP POLICY IF EXISTS.

BEGIN;

-- ── Layer 1: workflow trigger (master paid lock) ─────────────────────────────
...
BEGIN
  ...
  -- ── Master admin — pay + reopen (C3 paid lock) ────────────────────────────
  IF public.auth_user_is_master_admin() THEN

    IF OLD.status = 'paid'::public.settlement_status THEN
      IF NEW.status = 'paid'::public.settlement_status THEN
        RAISE EXCEPTION 'Master cannot modify paid settlement';
      END IF;
...
-- ── Layer 2: master RLS — non-paid operational + paid reopen only ───────────
...
--   AND policyname LIKE 'settlements_master%'
-- ORDER BY policyname;
