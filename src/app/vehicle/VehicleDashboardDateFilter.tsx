...
import type { VehicleDashboardDateFilter } from '@/lib/vehicle/vehicle-list'
import {
  buildVehicleDashboardHref,
  VEHICLE_DASHBOARD_ALL_RANGE_WARNING,
  VEHICLE_DASHBOARD_DEFAULT_RANGE_NOTICE,
  vehicleDashboardQuickRangeUrls,
} from '@/lib/vehicle/vehicle-list'

interface Props {
  filter: VehicleDashboardDateFilter
...
export function VehicleDashboardDateFilterBar({
  filter,
  showDefaultMonthNotice,
  showAllWarning,
}: Props) {
  const router = useRouter()
  const quick = vehicleDashboardQuickRangeUrls()
  const [from, setFrom] = useState(filter.from ?? '')
  const [to, setTo] = useState(filter.to ?? '')

  const applyCustomRange = () => {
    router.push(buildVehicleDashboardHref(from || null, to || null))
  }
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end">
        <label className="flex flex-col gap-1 text-xs text-gray-600">
          ...
          <input
            ...
            className="min-h-10 rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-orange-500"
          />
        </label>
        <label className="flex flex-col gap-1 text-xs text-gray-600">
          종료일
